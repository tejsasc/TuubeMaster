...
const initialState = {
  ...
  remarks: "",
  COS: [],
  calibration_orifice_set: null,
  calibration_orifice_setName: "",
  isEdit: false,
  projectId: "",
};
...
const CalibrationRackForm = (props) => {
  ...
  const getformData = function () {
    ...
    appendData(
      ...
      formData.calibration_orifice_set
    );
  ...
  const sendData = async function (e) {
    ...
    if (props.data.isEdit) {
      url = `/eq/calibrationretupddel/${props.editId}/`;
      method = "PATCH";
    } else {
      url = "/eq/calibrationcreate/";
      method = "POST";
    }
    ...
    if (response.status === 201) {
      dispatch(
        uiAction.setNotification({
          ...
          msg: `Calibration Rack ${formData.alternate_name} Added Successfully`,
        })
      );
      ...
      dispatch(
        uiAction.setNotification({
          show: true,
          heading: "Calibration Rack",
          msg: `Calibration Rack ${formData.alternate_name} Edited Successfully`,
        })
      );
      dispatchInputChange({ reset: true });
      props.refresh();
  ...
  const getCRData = async function () {
    setIsLoading(true);
    // console.log("it's", props.editId);
    const response = await ajaxCall(
      `/eq/calibrationretupddel/${props.editId}/`,
      {
        headers: {
    ...
    dispatchInputChange({
      ...
      data: {
        ...
        calibration_orifice_set: response.data?.calibration_orifice_set,
    ...
    <>
            ...
            {props.data.isEdit && isLoading ? (
              ...
              <Form onSubmit={sendData}>
                ...
                >
                  ...
                  <Tab eventKey={1} title="Calibration Rack Info">
                    <div className="row">
                      <Form.Group
                        className="mb-3 col-md-6"
                  ...
                  <Tab eventKey={2} title="Specification">
                    <div className="row">
                      <SingleSelection
                        ...
                        label="Calibration Orifice Set"
                        isNeed={props.data.isEdit}
                        separator="?"
                        paramId={props.editId}
                        paramName="cr_id"
                        value={formData.calibration_orifice_set}
                        onChange={(val) => {
                          dispatchInputChange({
                            type: "calibration_orifice_set",
                            value: val,
                          });
                        }}
                        isSearch={true}
                        url={
                          props.isWhEdit
                            ? `/get/option/calibrationorificepart/?warehouse_id=${props.whId}`
                            : `/get/option/calibrationorificepart/${
                        ...
                        setPopupthing={(value) => {
                          dispatchInputChange({
                            type: "calibration_orifice_setName",
                            value,
                          });
                        }}
                      />
                ...
                {formData.isEdit ? (
                  <>
                    <p className="text-center">
                      Can't Edit Calibration Rack, As Calibration Rack is used
                      in Project that is currently going on,
                    </p>
                    <p className="text-center">
                      <Link
      ...
      {isShowPopup ? (
        <EquipPartModal
          ...
          title="Want to Save Calibration Rack ?"
          body={
            <div className="row">
              <div className="col-md-6 grid3Auto rowGap20">
                ...
                <span>Calibration Orifice Set</span>:{" "}
                <span>{formData?.calibration_orifice_setName}</span>
                <span>Warehouse</span> :{" "}
...
export default CalibrationRackForm;
