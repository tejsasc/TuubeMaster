...
const TTDDatatable = (props) => {
  ...
  const getEquip = async () => {
    ...
    let url = `/eq/listttd/?ordering=-created_at&p=${pageNo}&records=${perPage}${filterUrl}`;
    if (props?.warehouse) {
    ...
    console.log("---response--0909090-->", response);
  ...
  const dismentalEquipPopup = function (id, name, part1, part2, part3) {
    setpopup(
      <DismentalPopup
        ...
        setpopup={setpopup}
        data={[
          { name: "Pressure Sensor", val: part1 },
          { name: "TTD Tube Seal Rack", val: part2 },
          { name: "Supply Orifice", val: part3 },
        ]}
      />
    );
  };
  ...
  const dismentalEquip = async function (id, name) {
    let url = `/eq/ttdretupddel/${id}/`,
      method = "PATCH";
    const data = new FormData();
    data.append("supply_orifice_set", "");
    data.append("pressure_sensor", "");
    data.append("TTD_tube_seal_rack", "");
    const response = await ajaxCall(
      url,
      {
        headers: {
    ...
    if (response.status === 200) {
      dispatch(
        uiAction.setNotification({
          show: true,
          heading: "TTD",
          msg: `TTD ${name} Dismantled Successfully`,
        })
      );
      setpopup();
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <>
          {row.supply_orifice_set ||
          row.pressure_sensor ||
          row.TTD_tube_seal_rack ? (
            <button
    ...
    },
    {
      ...
      //   //   rootClose
      //   // >
      //   //   <span onClick={changePMStatus.bind(null, row.id)}>row.pm_status</span>
      //   // </OverlayTrigger>
      // ),
      selector: (row) => (
        <PopOverPMstatus
    ...
    {
      name: "Supply Orifice Set",
      selector: (row) => row.supply_orifice_set,
    },
    {
      name: "Pressure Sensor",
      selector: (row) => row.pressure_sensor,
    },
    {
      name: "TTD TSR",
      selector: (row) => row.TTD_tube_seal_rack,
    },
    {
      name: "Remarks",
  ...
  return (
    <div className="col-md-12">
      <Card className="mt-3">
        <Card.Body>
          <Card.Title className="mb-3">All TTD</Card.Title>
          <Filters
