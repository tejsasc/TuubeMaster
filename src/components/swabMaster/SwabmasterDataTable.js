...
const SwabMasterDatabTable = (props) => {
  ...
  const getEquip = async () => {
    ...
    let url = `/eq/swabmasterlist/?ordering=-created_at&p=${pageNo}&records=${perPage}${filterUrl}`;
    if (props?.warehouse) {
  ...
  const dismentalEquipPopup = function (id, name, part1) {
    setpopup(
      <DismentalPopup
        ...
        name={name}
        setpopup={setpopup}
        data={[{ name: "Swab Master Tube Seal Rack", val: part1 }]}
      />
    );
  };
  const dismentalEquip = async function (id, name) {
    let url = `/eq/swabmasterretupddel/${id}/`,
      method = "PATCH";
    const data = new FormData();
    data.append("Swab_Master_Tube_Seal_Rack", "");
    const response = await ajaxCall(
      url,
      {
        headers: {
    ...
    if (response.status === 200) {
      dispatch(
        uiAction.setNotification({
          show: true,
          heading: "Swab Master",
          msg: `Swab Master ${name} Dismantled Successfully`,
        })
      );
      setpopup();
    }
  ...
  const columns = [
    {
      cell: (row) => (
        <>
          ...
          <Link
            to={`/swab-master/edit/${row.slug}`}
            className="projectAction"
            title="Edit Swab Master"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
    ...
    },
    {
      ...
      cell: (row) => (
        <>
          {row.Swab_Master_Tube_Seal_Rack ? (
            <button
    ...
    },
    {
      ...
      selector: (row) => (
        <PopOverPMstatus
          ...
          id={row.id}
          url={`/eq/swabmasterretupddel/${row.id}/`}
          name="TTD"
    ...
    {
      name: "Swab Master Tube seal rack",
      selector: (row) => row.Swab_Master_Tube_Seal_Rack,
    },
    {
      name: "Remarks",
  ...
  return (
    <>
      <Card className="mt-3">
        <Card.Body>
          <Card.Title className="mb-3">All Swab Master</Card.Title>
          <div>
            <Filters
...
export default SwabMasterDatabTable;
