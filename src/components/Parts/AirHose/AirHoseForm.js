...
const AirHoseForm = (props) => {
  ...
  const getformData = function () {
    ...
    appendData(data, "warehouse", formData.location_for_warehouse);
  ...
  const sendData = async function (e) {
    ...
    if (props.data.isEdit) {
      url = `/part/airhoseretupddel/${props.editId}/`;
      method = "PATCH";
    } else {
      url = "/part/airhosecreate/";
      method = "POST";
    }
    ...
    if (response.status === 201) {
      dispatch(
        uiAction.setNotification({
          show: true,
          heading: "Air Hose",
          msg: `Air Hose ${formData.alternate_name} Added Successfully`,
        })
      );
      ...
      dispatch(
        uiAction.setNotification({
          show: true,
          heading: "Air Hose",
          msg: `Air Hose ${formData.alternate_name} Edited Successfully`,
        })
      );
      dispatchInputChange({ reset: true });
      props.refresh();
  ...
  const getAirHoseData = async function () {
    ...
    const response = await ajaxCall(
      `/part/airhoseretupddel/${props.editId}/`,
      {
        headers: {
    ...
    dispatchInputChange({
      ...
      data: {
        ...
        wareHouse: [],
        location_for_warehouseName: "",
        location_for_warehouse: response.data?.warehouse,
            ...
            {props.data.isEdit && isLoading ? (
              ...
              <Form onSubmit={showPrompt}>
                ...
                >
                  ...
                  <Tab eventKey={1} title="Air Hose Info">
                    <div className="row">
                      <Form.Group
                        className="mb-3 col-md-6"
                ...
                <div className="col-md-12 flexCenter gap10">
                  ...
                  {props.data.isEdit ? (
                    <Link to="/airhose" className="btn btn-secondary">
                      Cancel Edit
                    </Link>
                  ) : (
                    ""
          ...
          body={
            <div className="row">
              <div className="col-md-6 grid3Auto rowGap20">
                ...
                <span>Colour code</span> : <span>{formData?.colorCode}</span>
                <span>Breadth </span>:{" "}
                <span>
