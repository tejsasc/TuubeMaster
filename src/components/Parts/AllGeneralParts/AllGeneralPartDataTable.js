...
const AllGeneralPartDatabTable = (props) => {
  ...
  const getEquip = async () => {
    ...
    let url = `/part/allgeneralpartlist/?ordering=-created_at&p=${pageNo}&records=${perPage}&date=${searchDate}${filterUrl}`;
    if (props?.warehouse) {
  ...
  const columns = [
    {
      cell: (row) => (
        <>
          ...
          <Link
            to={`/general-parts/edit/${row.slug}`}
            className="projectAction"
            title="Edit Genral Part"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
    ...
    },
    {
      ...
      selector: (row) => (
        <PopOverPMstatus
          ...
          id={row.id}
          url={`/part/allgeneralpartretupddel/${row.id}/`}
          name="General Part"
  ...
  return (
    <Card className="mt-3">
      <Card.Body>
        <Card.Title className="mb-3">All General Part</Card.Title>
        <div>
          <Filters
...
export default AllGeneralPartDatabTable;
