...
const BDDTSRDatabTable = (props) => {
  ...
  const getEquip = async () => {
    ...
    let url = `/part/bddtubesealracklist/?ordering=-created_at&p=${pageNo}&records=${perPage}${filterUrl}`;
    if (props?.warehouse) {
    ...
    },
    {
      ...
      selector: (row) => (
        <PopOverPMstatus
          ...
          id={row.id}
          url={`/part/calibrationorificeretupddrl/${row.slug}/`}
          name="BDD TSR"
  ...
  return (
    <Card className="mt-3">
      <Card.Body>
        <Card.Title className="mb-3">All BDD Tube Seal Rack</Card.Title>
        <div>
          <Filters
...
export default BDDTSRDatabTable;
