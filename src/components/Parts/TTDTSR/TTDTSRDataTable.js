...
const TTDTSRDatabTable = (props) => {
  ...
  const getEquip = async () => {
    ...
    let url = `/part/tddtubesealracklist/?ordering=-created_at&p=${pageNo}&records=${perPage}${filterUrl}`;
    if (props?.warehouse) {
    ...
    },
    {
      ...
      selector: (row) => (
        <PopOverPMstatus
          ...
          id={row.id}
          url={`/part/tddtubesealrackretupddrl/${row.id}/`}
          name="TTD TSR"
  ...
  return (
    <Card className="mt-3">
      <Card.Body>
        <Card.Title className="mb-3">All TTD Tube Seal Rack</Card.Title>
        <div>
          <Filters
...
export default TTDTSRDatabTable;
