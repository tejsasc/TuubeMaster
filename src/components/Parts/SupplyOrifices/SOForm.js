...
const SOForm = (props) => {
  ...
  const getformData = function () {
    ...
    appendData(data, "size", formData.size);
    appendData(data, "total_sets", formData.totalSets);
    appendData(data, "orifice_in_each_set", formData.OrificeInEachSet);
    appendData(data, "storage_case", formData.storageCase);
    return data;
  };
  ...
  const sendData = async function (e) {
    ...
    if (props.data.isEdit) {
      url = `/part/supplyorificeretupddrl/${props.editId}/`;
      method = "PATCH";
    } else {
      url = "/part/supplyorificecreate/";
      method = "POST";
    }
    ...
    if (response.status === 201) {
      dispatch(
        uiAction.setNotification({
          show: true,
          heading: "Supply Orifice",
          msg: `Supply Orifice ${formData.alternate_name} Added Successfully`,
        })
      );
      ...
      dispatch(
        uiAction.setNotification({
          show: true,
          heading: "Supply Orifice",
          msg: `Supply Orifice ${formData.alternate_name} Edited Successfully`,
        })
      );
      dispatchInputChange({ reset: true });
      props.refresh();
  ...
  const getAirHoseData = async function () {
    ...
    const response = await ajaxCall(
      `/part/supplyorificeretupddrl/${props.editId}/`,
      {
        headers: {
    ...
    dispatchInputChange({
      ...
      data: {
        ...
        totalSets: response.data?.total_sets,
        OrificeInEachSet: response.data?.orifice_in_each_set,
        storageCase: response.data?.storage_case,
      },
    });
    ...
    <>
            ...
            {props.data.isEdit && isLoading ? (
              ...
              <Form onSubmit={showPrompt}>
                ...
                >
                  ...
                  <Tab eventKey={1} title="Supply Orifice Info">
                    <div className="row">
                      <Form.Group
                        className="mb-3 col-md-6"
                  ...
                  <Tab eventKey={2} title="Specification">
                    <div className="row">
                      ...
                      >
                        ...
                        <Form.Control
                          type="text"
                          value={formData.OrificeInEachSet}
                          onChange={(e) =>
                            dispatchInputChange({
                              type: "OrificeInEachSet",
                              value: e.target.value,
                            })
                          }
                        />
                ...
                <div className="col-md-12 flexCenter gap10">
                  ...
                  {props.data.isEdit ? (
                    <Link to="/supply-orifices" className="btn btn-secondary">
                      Cancel Edit
                    </Link>
                  ) : (
                    ""
      ...
      {isShowPopup ? (
        <EquipPartModal
          ...
          title="Want to Save Supply Orifice ?"
          body={
            <div className="row">
              <div className="col-md-6 grid3Auto rowGap20">
                ...
                <span>Size</span>:<span>{formData?.size}</span>{" "}
                <span>Orifice in each set</span>:
                <span>{formData?.OrificeInEachSet}</span>
                <span>Storage Case</span>:<span>{formData?.storageCase}</span>
              </div>
              <div className="col-md-6 grid3Auto rowGap20">
                ...
                <span>Total Sets</span>:<span>{formData?.totalSets}</span>
              </div>
