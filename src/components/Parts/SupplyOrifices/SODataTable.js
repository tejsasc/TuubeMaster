...
const SODatabTable = (props) => {
  ...
  const getEquip = async () => {
    ...
    let url = `/part/supplyorificelist/?ordering=-created_at&p=${pageNo}&records=${perPage}${filterUrl}`;
    if (props?.warehouse) {
  ...
  const columns = [
    {
      cell: (row) => (
        <>
          ...
          <Link
            to={`/supply-orifices/edit/${row.slug}`}
            className="projectAction"
    ...
    },
    {
      ...
      selector: (row) => (
        <PopOverPMstatus
          ...
          id={row.id}
          url={`/part/supplyorificeretupddrl/${row.id}/`}
          name="Supply Orifice"
  ...
  return (
    <Card className="mt-3">
      <Card.Body>
        <Card.Title className="mb-3">All Supply Orifice</Card.Title>
        <div>
          <Filters
