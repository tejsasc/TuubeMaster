...
const DHDatabTable = (props) => {
  ...
  const getEquip = async () => {
    ...
    let url = `/part/devicehoselist/?ordering=-created_at&p=${pageNo}&records=${perPage}${filterUrl}`;
    if (props?.warehouse) {
    ...
    },
    {
      ...
      selector: (row) => (
        <PopOverPMstatus
          ...
          id={row.id}
          url={`/part/devicehoseretupddel/${row.id}/`}
          name="Device Hose"
  ...
  return (
    <Card className="mt-3">
      <Card.Body>
        <Card.Title className="mb-3">All Device Hose</Card.Title>
        <div>
          <Filters
