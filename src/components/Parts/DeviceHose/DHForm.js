...
const DHForm = (props) => {
  ...
  const sendData = async function (e) {
    ...
    if (props.data.isEdit) {
      url = `/part/devicehoseretupddel/${props.editId}/`;
      method = "PATCH";
    } else {
      url = "/part/devicehosecreate/";
      method = "POST";
    }
    ...
    if (response.status === 201) {
      dispatch(
        uiAction.setNotification({
          show: true,
          heading: "Device Hose",
          msg: `Device Hose ${formData.alternate_name} Added Successfully`,
        })
      );
      ...
      dispatch(
        uiAction.setNotification({
          show: true,
          heading: "Device Hose",
          msg: `Device Hose ${formData.alternate_name} Edited Successfully`,
        })
      );
      dispatchInputChange({ reset: true });
      props.refresh();
  ...
  const getAirHoseData = async function () {
    ...
    const response = await ajaxCall(
      `/part/devicehoseretupddel/${props.editId}/`,
      {
        headers: {
    ...
    <>
            ...
            {props.data.isEdit && isLoading ? (
              ...
              <Form onSubmit={showPrompt}>
                ...
                >
                  ...
                  <Tab eventKey={1} title="Device Hose Info">
                    <div className="row">
                      <Form.Group
                        className="mb-3 col-md-6"
                ...
                <div className="col-md-12 flexCenter gap10">
                  ...
                  {props.data.isEdit ? (
                    <Link to="/devicehose" className="btn btn-secondary">
                      Cancel Edit
                    </Link>
                  ) : (
                    ""
      ...
      {isShowPopup ? (
        <EquipPartModal
          ...
          title="Want to Save Device Hose ?"
          body={
            <div className="row">
