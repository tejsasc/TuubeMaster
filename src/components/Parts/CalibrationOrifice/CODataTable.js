...
const CODatabTable = (props) => {
  ...
  const getEquip = async () => {
    ...
    let url = `/part/calibrationorificelist/?ordering=-created_at&p=${pageNo}&records=${perPage}${filterUrl}`;
    if (props?.warehouse) {
  ...
  const columns = [
    {
      cell: (row) => (
        <>
          ...
          <Link
            to={`/calibration-orifices/edit/${row.slug}`}
            className="projectAction"
            title="Edit Calibration Ofices"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
    ...
    },
    {
      ...
      selector: (row) => (
        <PopOverPMstatus
          ...
          id={row.id}
          url={`/part/bddtubesealrackretupddrl/${row.id}/`}
          name="BDD TSR"
  ...
  return (
    <Card className="mt-3">
      <Card.Body>
        <Card.Title className="mb-3">All Calibration Orifice</Card.Title>
        <div>
          <Filters
