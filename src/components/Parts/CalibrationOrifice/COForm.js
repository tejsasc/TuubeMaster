...
const COForm = (props) => {
  ...
  const getformData = function () {
    ...
    appendData(data, "in_sets", formData.inSets);
    return data;
  };
  ...
  const sendData = async function (e) {
    ...
    if (props.data.isEdit) {
      url = `/part/calibrationorificeretupddrl/${props.editId}/`;
      method = "PATCH";
    } else {
      url = "/part/calibrationorificecreate/";
      method = "POST";
    }
    ...
    if (response.status === 201) {
      dispatch(
        uiAction.setNotification({
          show: true,
          heading: "Calibration Orifice",
          msg: `Calibration Orifice ${formData.alternate_name} Added Successfully`,
        })
      );
      ...
      dispatch(
        uiAction.setNotification({
          show: true,
          heading: "Calibration Orifice",
          msg: `Calibration Orifice ${formData.alternate_name} Edited Successfully`,
        })
      );
      dispatchInputChange({ reset: true });
  ...
  const getAirHoseData = async function () {
    ...
    const response = await ajaxCall(
      `/part/calibrationorificeretupddrl/${props.editId}/`,
      {
        headers: {
    ...
    <>
            ...
            {props.data.isEdit && isLoading ? (
              ...
              <Form onSubmit={showPrompt}>
                ...
                >
                  ...
                  <Tab eventKey={1} title="Calibration Orifice Info">
                    <div className="row">
                      <Form.Group
                        className="mb-3 col-md-6"
                  ...
                  <Tab eventKey={2} title="Specification">
                    <div className="row">
                      ...
                      <Form.Group
                        ...
                        controlId="formGroupRemark"
                      >
                        <Form.Label>In Sets</Form.Label>
                        <Form.Control
                          type="text"
                          value={formData.inSets}
                          onChange={(e) =>
                            dispatchInputChange({
                              type: "inSets",
                              value: e.target.value,
      ...
      {isShowPopup ? (
        <EquipPartModal
          ...
          title="Want to Save Calibration Orifice?"
          body={
            <div className="row">
              <div className="col-md-6 grid3Auto rowGap20">
                ...
                <span>In Sets </span>: <span>{formData?.inSets}</span>
              </div>
              <div className="col-md-6 grid3Auto rowGap20">
