...
const SMTSRDatabTable = (props) => {
  ...
  const getEquip = async () => {
    ...
    let url = `/part/swabmastertsrlist/?ordering=-created_at&p=${pageNo}&records=${perPage}${filterUrl}`;
    if (props?.warehouse) {
    ...
    },
    {
      ...
      selector: (row) => (
        <PopOverPMstatus
          ...
          id={row.id}
          url={`/part/swabmastertsrretupddrl/${row.id}/`}
          name="Swab Master"
  ...
  return (
    <Card className="mt-3">
      <Card.Body>
        <Card.Title className="mb-3">All SwabMaster TSR</Card.Title>
        <div>
          <Filters
...
export default SMTSRDatabTable;
