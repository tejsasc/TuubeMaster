...
function PSForm(props) {
  ...
  const getformData = function () {
    ...
    appendData(data, "upload_file", formData.image);
    ...
    appendData(data, "range", formData.range);
    appendData(data, "quantity", formData.quantity);
    return data;
  };
  ...
  const sendData = async function (e) {
    ...
    if (props.data.isEdit) {
      url = `/part/pressuresensorretupddrl/${props.editId}/`;
      method = "PATCH";
    } else {
      url = "/part/pressuresensorcreate/";
      method = "POST";
    }
    ...
    if (response.status === 201) {
      dispatch(
        uiAction.setNotification({
          ...
          msg: `Pressure Sensor ${formData.alternate_name} Added Successfully`,
        })
      );
      ...
      dispatch(
        uiAction.setNotification({
          show: true,
          heading: "Pressure Sensor",
          msg: `Pressure Sensor ${formData.alternate_name} Edited Successfully`,
        })
      );
      dispatchInputChange({ reset: true });
      props.refresh();
  ...
  const getAirHoseData = async function () {
    ...
    const response = await ajaxCall(
      `/part/pressuresensorretupddrl/${props.editId}/`,
      {
        headers: {
    ...
    <>
            ...
            {props.data.isEdit && isLoading ? (
              ...
              <Form onSubmit={showPrompt}>
                ...
                >
                  ...
                  <Tab eventKey={1} title="Pressure Sensor Info">
                    <div className="row">
                      <Form.Group
                        className="mb-3 col-md-6"
                ...
                <div className="col-md-12 flexCenter gap10">
                  ...
                  {props.data.isEdit ? (
                    <Link to="/pressure-sensor" className="btn btn-secondary">
                      Cancel Edit
                    </Link>
                  ) : (
                    ""
      ...
      {isShowPopup ? (
        <EquipPartModal
          ...
          title="Want to Save Pressure Sensor ?"
          body={
            <div className="row">
              <div className="col-md-6 grid3Auto rowGap20">
                ...
                <span>Range</span>:<span>{formData?.range}</span>
              </div>
              <div className="col-md-6 grid3Auto rowGap20">
                ...
                <span>Quantity</span>:<span>{formData?.quantity}</span>
              </div>
