...
const PSDatabTable = (props) => {
  ...
  const getEquip = async () => {
    ...
    let url = `/part/pressuresensorlist/?ordering=-created_at&p=${pageNo}&records=${perPage}${filterUrl}`;
    if (props?.warehouse) {
  ...
  const columns = [
    {
      cell: (row) => (
        <>
          ...
          <Link
            to={`/pressure-sensor/edit/${row.slug}`}
            className="projectAction"
    ...
    },
    {
      ...
      selector: (row) => (
        <PopOverPMstatus
          ...
          id={row.id}
          url={`/part/pressuresensorretupddrl/${row.id}/`}
  ...
  return (
    <Card className="mt-3">
      <Card.Body>
        <Card.Title className="mb-3">All Pressure Sensors</Card.Title>
        <div>
          <Filters
