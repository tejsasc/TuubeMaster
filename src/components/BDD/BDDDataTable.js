...
const BDDDataTable = (props) => {
  ...
  const getEquip = async () => {
    ...
    let url = `/eq/bddlist/?ordering=-created_at&p=${pageNo}&records=${perPage}${filterUrl}`;
    if (props?.warehouse) {
  ...
  const dismentalEquipPopup = function (id, name, part1) {
    setpopup(
      <DismentalPopup
        ...
        name={name}
        setpopup={setpopup}
        data={[{ name: "BDD Tube Seal Rack", val: part1 }]}
      />
    );
  };

  const dismentalEquip = async function (id, name) {
    let url = `/eq/bddretupddel/${id}/`,
      method = "PATCH";
    const data = new FormData();
    data.append("BDD_tube_seal_rack", "");
    const response = await ajaxCall(
      url,
      {
        headers: {
    ...
    if (response.status === 200) {
      dispatch(
        uiAction.setNotification({
          show: true,
          heading: "BDD",
          msg: `BDD ${name} Dismantled Successfully`,
        })
      );
      setpopup();
    }
  ...
  const columns = [
    ...
    {
      name: "BDD tube seal rack",
      selector: (row) => row.BDD_tube_seal_rack,
    },
    {
      name: "Remarks",
  ...
  return (
    <>
      <Card className="mt-3">
        <Card.Body>
          <Card.Title className="mb-3">All BDD</Card.Title>
          <div>
            <Filters
