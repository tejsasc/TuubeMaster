...
const AllParts = (props) => {
  ...
  const getEquip = async () => {
    ...
    let url = `/wp/?p=${pageNo}&records=${perPage}&date=${searchDate}${filterUrl}`;
    if (props?.warehouse) {
          ...
          <Link
            to={
              "TTD_tube_seal_rack" in row
                ...
                ? `/calibration-stand/edit/${row.id}`
                : "Swab_Master_Tube_Seal_Rack" in row
                ? `/swab-master/edit/${row.id}`
                : ""
            }
            className="projectAction"
            title="Edit Air Hose"
  ...
  return (
    <Card className="mt-3">
      <Card.Body>
        <Card.Title className="mb-3">All Parts</Card.Title>
        <div>
          <Filters
